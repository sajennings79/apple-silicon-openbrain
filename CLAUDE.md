...
- **Enrichment**: Calls mlx-lm server (port 8000) running `Qwen3-30B-A3B-4bit` to auto-extract summary, tags, and entities from stored memories. Fire-and-forget after store/update.
...
- **Qwen3 think blocks**: Enrichment strips `<think>...</think>` from Qwen3 responses. The enrichment prompt uses `/no_think` prefix.
